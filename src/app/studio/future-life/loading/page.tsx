...
function FutureLifeLoadingContent() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      router.push(`/studio/future-life/result?id=${id}`)
    }, 600)
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-br from-violet-950 via-cyan-950 to-slate-900 flex items-center justify-center p-6">
      ...
      <div className="max-w-md w-full relative">
        ...
        <div className="mt-6">
          <p className="text-cyan-200/50 text-xs text-center mb-3">예상 소요 시간: 20-40초</p>
          <motion.div
            key={tipIndex}
          ...
          >
            <p className="text-cyan-100/90 text-xs text-center leading-relaxed">💡 {tips[tipIndex]}</p>
          </motion.div>
        </div>
      </div>
    </div>
